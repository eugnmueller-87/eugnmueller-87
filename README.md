...
**What I built.** A data lake over 19 feeds (ERP, warehouse, customer portal, service desk, returns, refurbisher, recommerce, finance) with a strict ingest (deduplicated deliveries, an unresolved table that never guesses a key, a per-serial timestamp chain), a device ledger, a residual-value forecaster, and seven levers with owners and decision rules. On top: one page with seven tabs for a head of procurement, from realisation curves per model series to a per-device calculator that says from which month a contract has recovered its costs. Since no provider's data is public, a 5,000-serial simulation drawn from the real catalogue stands in; every assumption is a labelled placeholder with an owner, and the feeds are the interface a client's data lake plugs into.
...
**See it:** [Restwert Engine repo](https://github.com/eugnmueller-87/restwert-engine)
