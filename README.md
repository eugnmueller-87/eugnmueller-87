...
**The approach.** Split on the trust boundary, not on convenience. **Two instances, one codebase**, and the split is drawn at the *retrieval engine*. The embedder and the hybrid index are gated at **registration time**, not at runtime: the tool is never offered to the model at all unless `ICARUS_BRAIN_MCP=1`, which only the machine I own sets. That distinction matters, a disabled feature is a config checkbox somebody can flip, an unregistered one has no code path to enable. So the **index, the embeddings and every vector never leave the box**. Note *text* is a separate question with a separate answer: the hosted instance can read the vault, but only through an authenticated private-repository path with a **read-only** credential, and the same read-only rule binds the local jobs, none of them can write back to the knowledge base. Being precise about which of those two boundaries you are claiming is the whole exercise.
...
- **Hybrid retrieval** over the vault, vector + BM25 + **RRF fusion**. **872 notes** indexed locally tonight, alongside 3,043 tag edges and 1,081 vector chunks, all embedded on this laptop.
...
**Reliability, four `launchd` services, each solving a different failure.**
...
- **`de.brain.integrity`:** a daily read-only audit of the knowledge graph itself, broken links, orphaned notes, label violations, pushed **only when the finding-set changes**. The detail I like best: the script lives *inside the vault repo*, so it reaches the machine through the same nightly pull as the data. One deploy path, not two. A second path is one you eventually forget to use.

**The failure I did not design for, and the instrument that now catches it.** Two instances that look identical become a liability the moment one stops updating. The hosted one redeploys on every push; the one I own updates only when I pull. On 11 August my phone was still pointed at the local instance, running **seven-day-old code**. Five health checks reported the hosted side green while the phone kept answering from the old build, including a flat "I can't send you files", which on *that* machine was true. Several commits went into repairing a host that was not the one answering.

The fix is not discipline, it is instrumentation. `/health` now returns the running commit **and a fingerprint of the fully assembled system prompt**, so "is my change live" is one request instead of an assumption. The tool inventory alone was not enough, and that is the transferable part: **it does not move when a prompt rule changes**, and from the user's seat an outdated instruction is indistinguishable from a missing capability. If you ship agents, version the prompt like you version the code, and expose it.
...
**The result.** An assistant that reads my live private knowledge base, runs 24/7 on a seven-year-old laptop, carries **no hosting bill and no per-query cost for embeddings or retrieval**, is reachable from my phone anywhere in the world, self-restarts, self-refreshes its index nightly, and tells me when it's genuinely unwell. **No vendor owns the knowledge base or the index**, the notes live in a private repository I control and the retrieval layer never leaves the house.
