...
|---|---|---|
| 🧠 **TrueSpend** | AI-native procurement OS. n8n + Claude Sonnet 4.6 + PostgreSQL + React. 12 autonomous workflows, 28-table schema, full P2I lifecycle. Email-based login with role-gated access (procurement, IT, controlling, legal, user, admin). 4-agent compliance onboarding, DocuSign e-signature (JWT Grant, embedded signing), Grafana observability, Jira escalations. **DB security layer:** non-superuser PostgREST role (`truespend_app`), all status transitions enforced through SECURITY DEFINER RPCs — raw PATCH on `tickets.status` returns 403 at the database. Quality gate (24 checks), pre-commit secret scanning. | [GitHub](https://github.com/eugnmueller-87/TrueSpend) · [Live Demo](https://intake-production-84a0.up.railway.app) |
