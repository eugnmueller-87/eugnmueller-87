...
*Every build below started from a real pain I lived running procurement teams. The table is the short version: what it solves, what it does, what came out, and what it runs on. Engineering depth folds open underneath.*

| Project | What it solves | What it does | Result | Built with |
|---|---|---|---|---|
| 🔁 **[Restwert Engine](https://github.com/eugnmueller-87/restwert-engine)** | A Device-as-a-Service provider rents devices out on a guessed residual value: nobody can say per serial number whether a device earned money over its whole cycle, or which lever (channel, contract term, repair, purchase discount) lost it. | A residual-value and asset-P&L engine that closes the device loop per serial from purchase order to credit note, calibrates residual value on public used-market evidence, and puts seven levers next to it, each with a threshold and the role that owns it. | **429 public price points on 123 models**, 361 of them strictly matched to a priced variant, turned into realisation curves by family and manufacturer; a 5,000-serial simulation over 19 data feeds with a 95 % complete timestamp chain; per-device TCO (support and MDM operations allocated per billed month, flagged as estimates) and "costs recovered from month X" for every contract term; a forecast-quality tab measuring the forecast of record against realised prices, business view and channel-adjusted model view; 413 tests. | Python · DuckDB · pandas · pydantic · Streamlit · plotly |
| 🧠 **[TrueSpend](https://github.com/eugnmueller-87/TrueSpend)**<br>[demo](https://intake-production-84a0.up.railway.app) | Category managers drown in manual PR triage, scattered approvals across IT/legal/controlling, and spend nobody can see end-to-end. | An AI-native procurement OS running the full purchase-to-invoice lifecycle autonomously. | 17 autonomous workflows, intake→invoice hands-off, role-gated ops board, money-moves locked at the database layer. | n8n · Claude Sonnet 4.6 · PostgreSQL · React |
| 📦 **[SCM-Master](https://github.com/eugnmueller-87/SCM-Master)**<br>[demo](https://scm-master-production.up.railway.app) | Procurement, warehouse flow, and asset lifecycle live in disconnected systems: no single source of truth, no safe automation. | An AI-native supply-chain OS unifying all three, with an autonomous weekly purchasing run that a human still gates. | Auto-places demand-justified POs only at **≥0.90 confidence & <€200k**, proven by a 29-scenario agent-safety harness; CI-gated, twin-deployed. | FastAPI · SQLAlchemy 2.0 · Postgres · statsforecast · LightGBM |
| 📈 **[SCM Power BI Cockpit](https://github.com/eugnmueller-87/SCM-POWER-BI)**<br>[dashboard](https://scm-power-bi-production.up.railway.app/) | A non-technical CEO asks "should we invest in AI demand forecasting?" and gets hype, not a decision. | A consulting-case cockpit: live 7-tab web dashboard + Power BI report on the same data, backed by cited research. | A defensible **invest / wait / pilot** recommendation with WMAPE/Bias accuracy, should-cost & TCO, and a phased plan. | Node · Chart.js · Power BI · DAX |
| 🔴 **[SpendLens](https://github.com/eugnmueller-87/PROCUREMENT)**<br>[demo](https://procurement-production-f940.up.railway.app) | Raw vendor spend arrives as messy CSVs: no classification, no compliance view, no supplier intelligence a category manager can act on. | A 5-stage AI pipeline: map → clean → classify vendors → flag compliance → surface supplier intel, across 7 decision screens. | Upload a spend file, get a classified, compliance-scored, DD-linked view, hardened for public deployment. | React 18 · FastAPI · SQLite |
| 🤝 **[Negotiation Agent](https://github.com/eugnmueller-87/Negotiation-Agent)**<br>[live demo](https://web-production-a5b7b.up.railway.app/) | Supplier negotiations anchor on price alone, so buyers leave win-win trades on the table, and nobody trusts an LLM to hold a walk-away line. | A **deployed** negotiation tool where the LLM has zero authority: a deterministic engine makes every accept/counter/walk-away call and fixes the exact figures a reply may state. A violating draft is rejected and redrafted server-side before it's ever sent. Real Opus writes the buyer's emails live; you play either seat. **Walk in prepared:** upload a contract → it extracts the deal (price, expiry, licenses, NDA/DPA), researches the supplier via my **Hades** agent, and proposes how the intelligence should *shape the mandate* (give/hold/hedge) via a deterministic rule table a human approves, so the LLM never edits the envelope. | At matched buyer cost, logrolling leaves the supplier **+0.27 utility** vs splitting the difference, a runnable eval. 183 tests, mypy-strict, CI-green, live on Railway. | Claude Opus + Haiku · FastAPI · mypy strict |
| ☠️ **[Hades](https://github.com/eugnmueller-87/hades)** | Supplier due-diligence (sanctions, registry, ESG, LkSG/CSDDD) takes analysts 1–2 days per supplier. | An autonomous agent that screens a company across 6 risk sources in parallel and returns a scored verdict. | **Full risk report in under 2 minutes:** sanctions/registry/ESG/news → risk score 1–10 + Approve/Block. | LangGraph · Claude · OFAC/UN + NorthData |
| 🔍 **[Hermes](https://github.com/eugnmueller-87/hermes-agent)** | Supplier market intelligence arrives too late: you learn a key supplier is in trouble after it hurts you. | A continuous market-intelligence agent watching a curated supplier list across 5 signal sources. | Tracks **56 AI suppliers across 8 categories**, classifies + delta-tracks signals, feeds DD and the CM on demand. | Claude Haiku · Upstash Vector · 5 crawlers |
| 🏗 **[Triage Agent](https://github.com/eugnmueller-87/IRONHACK/tree/main/WEEK%204/LAB4)** | Purchase requests pile up waiting for a human to route, compliance-check, and chase suppliers. | An autonomous agent that triages every PR: value-routes, checks NDA/DPA/MSA via RAG, runs RFQ/RFP outreach, recommends an award. | 6 importable n8n workflows replacing the manual triage queue end-to-end. | n8n · RAG |
| 📊 **[Marketing Spend Stats](https://github.com/eugnmueller-87/IRONHACK/tree/main/Week%206/LAB%209/lab_statistical_analysis_eugen_mueller)** | A $500K budget is split across 7 channels on gut feel, not evidence. | A full statistical pipeline testing every channel-pair for real CPA difference. | All 14 CPA pairs significant post-FDR → an executive memo with a data-backed reallocation. | Python · Welch t-tests · BH-FDR · bootstrap |
| 🧪 **[LLM Eval Lab](https://github.com/eugnmueller-87/Rating-System-for-AI-Responses)** | "Is this AI answer actually good enough to trust for procurement compliance Q&A?", usually answered by vibes. | A LangSmith evaluation lab that scores answers objectively and A/B-compares models. | LLM-as-judge on correctness + completeness over a custom dataset; gpt-4o-mini vs gpt-4o compared on real numbers. | LangSmith · gpt-4o-mini vs gpt-4o |

<details>
<summary><b>🔧 Engineering depth (click to expand, for technical reviewers)</b></summary>

- **Restwert Engine:** four-layer lake: raw landing files per feed and delivery period → typed bronze with sha256-deduplicated deliveries and an `unresolved` table (a key that does not resolve is recorded, never guessed) → a silver device ledger with one booking line per cost and revenue, each carrying its source reference and an `is_estimate` flag → gold KPIs, cohorts and levers. Public market module: 429 used-price anchors and launch RRPs with URL and date per row, log-linear realisation curves per family and manufacturer with condition-grade offsets, strict variant matching (storage and RAM, no base-variant fallback), fit quality gated (at least 12 anchors over 6 months of model age). Simulation: 5,000 serials bought by model generation, four contract terms with a term-dependent rate factor, four resale channels with fee tables, early terminations, price-protection windows; the forecaster learns residual ratios per model and grade on a 120-month grid and flags every clipped value. Seven levers (channel choice, purchase discount, condition and repair, ageing, contract term, price protection, manufacturer mix), each computed per serial as actual minus a named reference from the provider's own fleet, with threshold, owner and decision rule; additivity against the ledger is checked on every run. Governance: the model advises, deterministic code decides, a named role owns every threshold; every placeholder is labelled in config with its owner; no figure in any page is typed, every number in a sentence comes from the generator. Built with a multi-agent workflow: read-only web researchers (URL and date per row), adversarial verifiers sampling claims, and a wording critique in three roles (head of procurement, CFO, recommerce) before anything ships; that critique caught two invented example figures the drafting model had rounded into the text.
...
<details>
<summary><b>📋 Case study: Restwert Engine, residual value for Device-as-a-Service, framed as a client engagement</b></summary>
...
**The problem.** A Device-as-a-Service provider buys a phone or laptop, rents it out for 12 to 48 months, takes it back and sells it. The rent is priced on an assumed residual value, but the residual is only known at the very end, per serial number, after refurbishment, channel fees and the days the device sat in stock. Most providers see the margin per contract, not per device, and cannot say which lever lost the money: the channel, the term, a repair that was not worth it, a purchase discount below the fleet's own benchmark.

**The approach.** Two sources of truth, never mixed. Public evidence first: launch RRPs and used-market prices per model, condition and age, every row with a URL and a date, turned into realisation curves; that is the plausibility check a provider's own forecast has to survive. Then the provider's own data: one ledger line per booking per serial, from purchase order to credit note, with source reference and an estimate flag, so every number on every page can be traced to a row in a landing file. The rule throughout: *the model advises, deterministic code decides, a named role owns every threshold.*

**What I built.** A data lake over 19 feeds (ERP, warehouse, customer portal, service desk, returns, refurbisher, recommerce, finance) with a strict ingest (deduplicated deliveries, an unresolved table that never guesses a key, a per-serial timestamp chain), a device ledger, a residual-value forecaster, and seven levers with owners and decision rules. On top: one page with nine tabs (report, device, realisation, forecast quality, TCO, cycle, levers, term, data) for a head of procurement, from realisation curves per model series to a per-device calculator that says from which month a contract has recovered its costs. Since no provider's data is public, a 5,000-serial simulation drawn from the real catalogue stands in; every assumption is a labelled placeholder with an owner, and the feeds are the interface a client's data lake plugs into.

**The result.** 429 public price points on 123 models; a closed device loop with a 95 % complete timestamp chain; per-device TCO split into purchase price and cost to sale; a term analysis that shows the first year of a device's life is the expensive one, so short contracts must carry a higher monthly rate or a second rental cycle; early terminations losing money at every term, which points at the customer contract, not the price list.

**What I'd do for a client.** Connect only the agreed feeds, read-only, on a delivery schedule → replace the placeholders with the provider's own thresholds, owner by owner → run the ledger against one quarter of real history and reconcile it with finance → then let the seven levers name where the money sits, with a role next to each.

**See it:** [Restwert Engine repo](https://github.com/eugnmueller-87/restwert-engine) and the [live page](https://claude.ai/artifact/35YqfY2paPWtkn6yD32U7m) (nine tabs, German, synthetic fleet on a public catalogue)
...
| 🗂️ **[ONE TEAM, ONE DREAM](https://github.com/eugnmueller-87/one-team-one-dream)**<br>*shared brain, six-person team* · private | Six people, six separate streams of information. The process stops at every handover while somebody digs the context back out of their own inbox, and when one of them leaves, the team loses that stream for good. | **Combines the information streams of the whole team into one place, so the process runs end to end instead of stopping at each desk.** The same idea as above, ported from one person to six, where everyone is a user and nobody touches a repo. **Each person** keeps their own raw notes and sees their own numbers first. **The lead** sees what needs support, not a ranking. **Leadership** sees progress and where the projects stand. Nothing is ever deleted or overwritten, only added to. | **MVP runs and is measured:** 89 self-test checks plus 58 unit tests green, all 36 role/view pairs served. **Not deployed:** a works agreement and a recording-consent decision come first. | Python · FastAPI · Markdown + git · MCP |
