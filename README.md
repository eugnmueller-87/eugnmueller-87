...
| 📦 **SCM-Master** | Supply-chain system unifying procurement, transit-warehouse flow, and full asset lifecycle. FastAPI + SQLAlchemy 2.0 + Pydantic 2 (SQLite→Postgres). Multi-sourcing core: `Product` spec decoupled from `ProductSupplier` (lead time, MOQ, contract price, preference rank) — re-sourcing a line is one FK repoint. Serial-tracked `Asset` followed end-to-end (RECEIVED → IN_STORAGE → DEPLOYED → DECOMMISSIONED → DISPOSED) with an unbroken provenance link back to its purchase-order line for end-to-end spend tracing. | [GitHub](https://github.com/eugnmueller-87/SCM-Master) |
